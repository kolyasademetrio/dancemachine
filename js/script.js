$(document).ready(function(){
	...
	/*--------------------------------- функция для more photos -------------------------*/
	(function(){

		var $linkMore = $('.photosPerformance__linkMore'),
			$rowWrap = $('.photosPerformance__rowWrap');

		$rowWrap.not(':nth-child(1), :nth-child(2)').hide();

		$linkMore.on('click', function(e){

			e.preventDefault();

			var rowVisible = $('.photosPerformance__rowWrap:visible');

			if ( rowVisible.last().next().is(':hidden') ) {

				rowVisible.last().next().show();
	...
	/*--------------------------------- функция для more photos End -------------------------*/



	/*--------------------------------- функция для structureGroup -------------------------*/
	(function(){

		var groups = {};

		$('.structureGroup__content').each(function() {
	...
	/*--------------------------------- функция для structureGroup End -------------------------*/
	...
	/*--------------------------------- функция для якоря -------------------------*/
		...
		function ancor (ancor, goal) {
			...
			$(ancor).on('click', function(e){
			  $('html,body').stop().animate({ scrollTop: $(goal).offset().top - $('.header').height()}, 2000);
			  e.preventDefault();
			});

		}

		ancor($('.mainScreen__link'), $('.promo'));
		ancor($('#photoNavLink'), $('.photosPerformance'));
		ancor($('#repertoirNavLink'), $('.repertoire'));
		ancor($('#riderNavLink, #contactsNavLink'), $('.contactsBlock'));
	...
	function getSticky(stickyElemId, ancorId, fixedClassName){

		var stickyElem = document.getElementById(stickyElemId),
			ancor = document.getElementById(ancorId),
			ancorSourceBottom = ancor.getBoundingClientRect().bottom + window.pageYOffset;

		window.onscroll = function() {
			if (stickyElem.classList.contains(fixedClassName) && window.pageYOffset < ancorSourceBottom) {
				stickyElem.classList.remove(fixedClassName);
			} else if (window.pageYOffset > ancorSourceBottom) {
				stickyElem.classList.add(fixedClassName);
			}
		};

	}

	getSticky('header', 'ancorForHeader', 'fixed');
	/*--------------------------------- функция для липкого меню End -----------------------------*/
